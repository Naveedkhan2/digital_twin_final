...
import { useScaledMotorData } from "@/hooks/useScaledMotorData";
import { useMotorDrive } from "@/context/MotorDriveContext";
import { VfdControlBar } from "@/components/VfdControlBar";
import { ArrowUp, ArrowDown, Gauge } from "lucide-react";
import { Separator } from "@/components/ui/separator";

const Index = () => {
  const { motorData, vibrationData, predictiveMaintenance, loading, error, lastUpdated } =
    useScaledMotorData();
  const { effectiveRpm, motorOn } = useMotorDrive();
    ...
    return (
      <DashboardLayout title="Motor Monitoring System">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg text-white/60">Loading data...</div>
        </div>
      </DashboardLayout>
    );
  }
  ...
  if (!motorData) {
    return (
      <DashboardLayout title="Motor Monitoring System">
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          <div className="text-lg text-white/70">Waiting for motor data</div>
          <div className="text-sm text-white/50">
            Add motor data under motor01/logs (or live_reading) in Firebase Realtime Database.
          </div>
        </div>
      </DashboardLayout>
    );
  ...
  return (
    <DashboardLayout title="Motor Monitoring System" lastUpdated={lastUpdated}>
      <VfdControlBar />
      {isStale && (
        <div className="mb-4 p-3 rounded-lg bg-amber-500/15 border border-amber-500/35 text-amber-100 text-sm">
          <strong>Values not updating?</strong> Confirm something is still writing fresh data to Firebase{" "}
          <code className="bg-black/30 px-2 py-0.5 rounded">motor01/live_reading</code> or{" "}
          <code className="bg-black/30 px-2 py-0.5 rounded">motor01/logs</code>.
        </div>
      )}
